...
public class AnnotationGen {
	...
	private void add(IAttributeNode node, ICodeWriter code) {
		...
		for (IAnnotation a : aList.getAll()) {
			...
			if (!aCls.startsWith(Consts.DALVIK_ANNOTATION_PKG) && !aCls.equals(Consts.OVERRIDE_ANNOTATION) && (cls.root().getArgs().isGenerateKotlinMetadata() || !aCls.equals(KotlinMetadataUtils.KOTLIN_METADATA_ANNOTATION))) {
				code.startLine();
				formatAnnotation(code, a);
			}
		}
